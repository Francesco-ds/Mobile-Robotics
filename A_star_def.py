...
def forza_attrattiva(map,i,j):
    f_att_x = -ka * (i - goal[0])
    f_att_y = -ka *(j-goal[1])
    f_att_norm = np.linalg.norm([f_att_x,f_att_y])
    return f_att_norm

#repulsive force based on paper formulas
def forza_repulsiva(map,i,j):
    f_rep_x = 0
    f_rep_y = 0
    
    #every obstacle square is counted separately, 2 close squares obstacles are gonna have overlapped forces making that stronger
    for obstacle in obstacles:
        q_qc_x = i-obstacle[0]
        q_qc_y = j-obstacle[1]
        dq = np.linalg.norm([q_qc_x,q_qc_y])
        dq_squared = dq ** 2
        f_rep_x += kb*((1/dq) - (1/do) ) * (1/dq_squared) * ((i - obstacle[0])/dq)
        f_rep_y += kb*((1/dq) - (1/do) ) * (1/dq_squared) * ((j - obstacle[1])/dq)
    
    f_rep_norm = np.linalg.norm([f_rep_x,f_rep_y])
    return f_rep_norm

#getting the h cost of a tile; this is the heuristic part of A*
def h_cost(map,i,j):
    attrattiva = forza_attrattiva(map,i,j)
    repulsiva = forza_repulsiva(map,i,j)
    
    #simply sum the forces (both norms)
    hcost = attrattiva + repulsiva
    return hcost

# I can move in diagonal only if the tiles which the robot has to pass through are free, otherwise i'd have a collision in the simulation space
